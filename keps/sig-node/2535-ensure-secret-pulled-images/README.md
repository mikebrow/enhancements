...
Kubelet will track a hash map for the credentials that were successfully used to pull an image. Per review request the hash map will be persisted as state for the kubelet image manager in alpha. See https://github.com/kubernetes/kubernetes/pull/114847 PR for detailed design / behavior documentation.

/var/lib/kubelet/image_manager_state
{
  "images": {
...
- For beta/ga we may revisit/replace the hash map state in kubelet design, with an extension to the CRI API for having the container runtime
